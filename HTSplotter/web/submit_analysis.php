...
if ($_POST['user_input'] == 0){
	...
	$user_dir = 'user_files/User_'.$user_string.'_'.$date_part.'_HTSplotter/';
	$response_array['user_input'] = $user_dir;
	...
	$command = 'python3 /var/www/html/scripts/web.py -f /var/www/html/'.$user_dir.' -b '.escapeshellarg($_POST['bio_rep']).' -n '.escapeshellarg($_POST['biorep_fn']).' -e '.escapeshellarg($_POST['exp_effect']).' -i '.escapeshellarg($_POST['info_readout']).' -r '.escapeshellarg($_POST['readout_unit']).' -u '.escapeshellarg('no').' -sh '.escapeshellarg($_ENV['S3_HOST']).' -sb '.escapeshellarg($_ENV['S3_BUCKET']).' -sa '.escapeshellarg($_ENV['S3_ACCESS_KEY']).' -ss '.escapeshellarg($_ENV['S3_SECRET_KEY']);
	$output = shell_exec($command);

	$errorfile = '';
	$error_array = glob($user_dir.'results/*_Errorfile.txt');
	...
	if (file_exists($errorfile) && filesize($errorfile) > 0) {
		...
		foreach ($exp_types as $key => $exp_type) {
			...
			$infofile = $infofile.trim($user_dir.'results/'.$name_str2[0].'_information.txt').",";
		}
